            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                ...
                <div className="text-sm text-primary-600">
                  {language === 'ar' ? 'من الساعة 2:00 مساءً' : 'From 2:00 PM'}
                </div>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  {language === 'ar' ? 'تسجيل دخول ذاتي متاح' : 'Self check-in available'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <BadgeCheck className="w-5 h-5 text-green-500" />
                <span className="text-sm text-gray-600">
                  {language === 'ar' ? 'موظف استقبال متواجد' : 'Reception staff available'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-green-500" />
                <span className="text-sm text-gray-600">
                  {language === 'ar' ? 'تسجيل خروج ذاتي متاح' : 'Self check-out available'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  {language === 'ar' 
                    ? 'تسجيل خروج متأخر متاح حسب التوفر' 
          ...
          <div className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-sm text-yellow-700">
                {language === 'ar'
                  ? 'استرداد 50% من المبلغ حتى 12 ساعة قبل موعد الوصول'
                  : '50% refund up to 12 hours before check-in'}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 p-3 bg-red-50 rounded-lg">
            ...
            <div>
              ...
              <div className="text-sm text-red-700">
                {language === 'ar'
                  ? 'خلال آخر 12 ساعة قبل موعد الوصول'
      ...
      <div className="bg-white rounded-lg border p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="space-y-2">
              {[
                { icon: Ban, label: { ar: 'التدخين في الغرف', en: 'Smoking in rooms' } },
                { icon: Ban, label: { ar: 'الحفلات', en: 'Parties' } },
            ...
            <div className="space-y-2">
              {[
                { 
                  icon: Info, 
                  label: { 
                    ar: 'مطلوب بطاقة هوية سارية المفعول', 
                    en: 'Valid ID required' 
                  } 
                },
                { 
                  icon: Info, 
                  label: { 
                    ar: 'تأمين نقدي مطلوب عند تسجيل الدخول', 
                    en: 'Cash deposit required at check-in' 
                  } 
                }
              ].map((item, index) => (
                <div key={index} className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              {[
                { 
                  icon: Percent, 
                  label: { 
                    ar: 'دفعة مقدمة: 20% عند الحجز', 
                    en: 'Deposit: 20% upon booking' 
                  } 
                },
                { 
                  icon: DollarSign, 
                  label: { 
                    ar: 'المبلغ المتبقي: عند تسجيل الدخول', 
                    en: 'Remaining balance: at check-in' 
                  } 
                }
              ].map((item, index) => (
                <div key={index} className="flex items-center gap-2">

...
export function HotelBookingModal({ isOpen, onClose, hotel, room }: HotelBookingModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = language === 'ar'
      ? `*حجز جديد*\n\n` +
        `*الفندق:* ${hotel.nameAr}\n` +
        `*الغرفة:* ${room.titleAr}\n` +
        `*السعر:* ${room.price} ${room.currency} / ليلة\n` +
        `*عدد الليالي:* ${nights}\n` +
        (isCouponValid ? `*الإجمالي قبل الخصم:* ${totalPrice} ${room.currency}\n` : '') +
        (isCouponValid ? `*كود الخصم:* ${formData.coupon.trim()}\n*الخصم:* ${discount} ${room.currency}\n` : '') +
        `*${isCouponValid ? 'الإجمالي بعد الخصم' : 'الإجمالي'}:* ${isCouponValid ? finalPrice : totalPrice} ${room.currency}\n\n` +
        `*معلومات الحجز:*\n` +
        `${bookingType === 'self' 
      ...
      : `*New Booking*\n\n` +
        ...
        (isCouponValid ? `*Total before discount:* ${totalPrice} ${room.currency}\n` : '') +
        (isCouponValid ? `*Coupon Code:* ${formData.coupon.trim()}\n*Discount:* ${discount} ${room.currency}\n` : '') +
        `*${isCouponValid ? 'Total after discount' : 'Total'}:* ${isCouponValid ? finalPrice : totalPrice} ${room.currency}\n\n` +
        `*Booking Details:*\n` +
        `${bookingType === 'self' 
      ...
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="p-3 sm:p-4 border-b sticky top-0 bg-white z-10 flex items-center justify-between">
          <h2 className="text-base sm:text-xl font-bold text-gray-900">
            {language === 'ar' ? 'حجز غرفة' : 'Book Room'}
          </h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              ...
              <label htmlFor="policies-agreement" className="text-xs sm:text-sm text-gray-600">
                {language === 'ar' 
                  ? 'أوافق على سياسات الفندق وشروط الحجز. ' 
                  : 'I agree to the hotel policies and booking terms. '
                }
                <button
                  type="button"
                  onClick={() => {

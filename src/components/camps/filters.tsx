...
import { TERMANAT_FEATURES, TERMANAT_TYPES, TERMANAT_PURPOSES } from '@/data/termanat';
import { useState } from 'react';
import { TermanatFilters } from '@/pages/camps';
...
const CURRENCIES = [
  { id: 'YER', labelAr: 'ريال يمني', labelEn: 'Yemeni Rial' },
  { id: 'SAR', labelAr: 'ريال سعودي', labelEn: 'Saudi Riyal' },
  { id: 'USD', labelAr: 'دولار أمريكي', labelEn: 'US Dollar' },
];
...
type FiltersProps = {
  filters: TermanatFilters;
  setFilters: (filters: TermanatFilters) => void;
  showFilters: boolean;
  setShowFilters: (show: boolean) => void;
};

export function TermanatFiltersComponent({ 
  filters, 
  setFilters, 
  showFilters, 
  setShowFilters 
  ...
  const handlePurposeChange = (purpose: string | null) => {
    setFilters({ ...filters, purpose: filters.purpose === purpose ? null : purpose });
  };
  ...
  const FiltersContent = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            placeholder={language === 'ar' ? 'ابحث عن طيرمانة...' : 'Search for a Termanah...'}
      ...
      <div>
        <h3 className="font-medium mb-4">
          {language === 'ar' ? 'نوع الطيرمانة' : 'Termanah Type'}
        </h3>
        <div className="space-y-2">
          {TERMANAT_TYPES.map((type) => (
            <button
              key={type.id}

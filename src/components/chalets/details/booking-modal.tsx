...
export function ChaletBookingModal({ isOpen, onClose, chalet, section }: BookingModalProps) {
  ...
  const calculateTotalPrice = () => {
    ...
    return section ? section.price[chalet.currency] : chalet.price * nights;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = language === 'ar'
      ? `*حجز جديد*\n\n` +
        `*الفندق:* ${chalet.titleAr}\n` +
        (section ? `*الغرفة:* ${section.titleAr}\n` : '') +
        `*السعر:* ${section ? section.price[chalet.currency] : chalet.price} ${chalet.currency} / ليلة\n` +
        `*عدد الليالي:* ${nights}\n` +
        (isCouponValid ? `*الإجمالي قبل الخصم:* ${totalPrice} ${chalet.currency}\n` : '') +
        (isCouponValid ? `*كود الخصم:* ${formData.coupon.trim()}\n*الخصم:* ${discount} ${chalet.currency}\n` : '') +
        `*${isCouponValid ? 'الإجمالي بعد الخصم' : 'الإجمالي'}:* ${isCouponValid ? finalPrice : totalPrice} ${chalet.currency}\n\n` +
        `*معلومات الحجز:*\n` +
        `${bookingType === 'self' 
        ...
        `*تاريخ المغادرة:* ${formatDate(formData.checkOut)}\n` +
        `*عدد البالغين:* ${formData.adults}\n` +
        `*عدد الأطفال:* ${formData.children}\n` +
        (formData.notes ? `\n*ملاحظات:*\n${formData.notes}` : '')
      : `*New Booking*\n\n` +
        `*Hotel:* ${chalet.titleEn}\n` +
        (section ? `*Room:* ${section.titleEn}\n` : '') +
        `*Price:* ${section ? section.price[chalet.currency] : chalet.price} ${chalet.currency} / night\n` +
        `*Number of nights:* ${nights}\n` +
        (isCouponValid ? `*Total before discount:* ${totalPrice} ${chalet.currency}\n` : '') +
        (isCouponValid ? `*Coupon Code:* ${formData.coupon.trim()}\n*Discount:* ${discount} ${chalet.currency}\n` : '') +
        `*${isCouponValid ? 'Total after discount' : 'Total'}:* ${isCouponValid ? finalPrice : totalPrice} ${chalet.currency}\n\n` +
        `*Booking Details:*\n` +
        `${bookingType === 'self' 
      ...
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="p-3 sm:p-4 border-b sticky top-0 bg-white z-10 flex items-center justify-between">
          <h2 className="text-base sm:text-xl font-bold text-gray-900">
            {language === 'ar' ? 'حجز شاليه' : 'Book Chalet'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-3 sm:p-4 bg-gray-50 border-b">
          ...
          <div className="text-primary-600 font-medium mt-1 text-sm sm:text-base">
            {section ? section.price[chalet.currency] : chalet.price} {chalet.currency}
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              ...
              <label htmlFor="policies-agreement" className="text-xs sm:text-sm text-gray-600">
                {language === 'ar' 
                  ? 'أوافق على سياسات الشاليه وشروط الحجز. ' 
                  : 'I agree to the chalet policies and booking terms. '
                }
                <button
                  type="button"
                  onClick={() => {
